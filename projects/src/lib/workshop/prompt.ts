...
10. **作答格式。** 选择/排序/匹配题由页面交互作答，送到你手上是纯文本：「选择：B」表示单选所选项；「多选：A、C」表示多选所选项；「排序：a → b → c①、c②」表示分层排序结果（「→」分隔层、左为先/上；「、」为同层并列，表示学生认为同层单位先后不确定）；「匹配：A→②；B→①」表示左列项与右列项的对应；「补充：」是选择/排序题附带的说明。引用这些作答时按此解读。
