...
10. **作答格式。** 选择/排序/匹配题由页面交互作答，送到你手上是纯文本：「选择：B」表示单选所选项；「多选：A、C」表示多选所选项；「排序：a → b → c①、c②」表示分层排序结果（「→」分隔层、左为先/上；「、」为同层并列，表示学生认为同层单位先后不确定）；「匹配：A→②；B→①」表示左列项与右列项的对应；「补充：」是选择/排序题附带的说明。画图题以图片形式提交，你看到的是学习者绘制的示意图。引用这些作答时按此解读。
...
export function describeInput(input: WjPartInput): string {
  switch (input.type) {
    ...
    case 'drawing':
      return `作答形式：画图（学习者在画板上绘制，提交后以图片判分）`;
  }
}
...
学习者此刻正读到本环节的「${actTitle}」一节（环节资料按 工序说明 → 关键数据 → 上机操作 → 细问 的顺序推进，同屏只显示当前这一节；缺哪一节就跳过哪一节）。
...
「上机操作」一节是虚拟仿真工作台：他在上面按真实工序排次序、挑工具、调参数、逐段推进，
每一步的后果由工作台当场给出，你不评分也不代他操作。他可能把一份操作记录贴进来
（形如「我刚在某某工作台上走了一遍，结果……」）——那时就按先答后评的规矩追问他：
让他自己说出哪一步代价最大、为什么，不要直接替他复盘出正确做法。`;
}
