/**
 * 演示学情数据（教师端一键注入/清除的唯一实现，scripts/seed-demo.mjs 与
 * /api/teacher/seed-demo 共用）。账号 email = 学工号@demo.invalid，幂等：
 * 账号按 email 复用、进度按 user_id upsert，重复注入只刷新数据与活跃时间。
 */
...
function styled(key: string, level: 'full' | 'mid' | 'low') {
  const raw = A[key];
  if (!raw) return '';
  ...
  const head = sents.slice(0, level === 'mid' ? 2 : 1).join('');
...
const STUDENTS: Array<[staffNo: string, name: string, profileKey: string, hoursAgo: number | null]> = [
  // 202501 班 · 标准梯度
  ['2025010101', '林知远', 'top', 2],
  ['2025010102', '沈清和', 'good', 21],
  ['2025010103', '顾晓帆', 'good', 9],
  ['2025010104', '周雨桐', 'solid', 50],
  ['2025010105', '陈默', 'mid', 74],
  ['2025010106', '韩霜', 'mid', 142],
  ['2025010107', '方启', 'low', 200],
  ['2025010108', '陆之遥', 'starter', 240],
  ['2025010109', '潘越', 'starter', 300],
  ['2025010110', '岳琳', 'none', null],
  // 202502 班 · 整体较好
  ['2025020101', '苏砚', 'top', 5],
  ['2025020102', '江晚晴', 'top', 30],
  ['2025020103', '罗一鸣', 'good', 12],
  ['2025020104', '谭星', 'good', 46],
  ['2025020105', '孟真', 'solid', 66],
  ['2025020106', '白露', 'solid', 120],
  ['2025020107', '程曦', 'mid', 176],
  ['2025020108', '邱荃', 'low', 210],
  ['2025020109', '闻人悦', 'starter', 260],
  ['2025020110', '章含', 'none', null],
  // 202503 班 · 整体偏弱
  ...
  ['2025030106', '岑露', 'low', 160],
  ['2025030107', '应川', 'starter', 190],
  ['2025030108', '齐若飞', 'starter', 230],
  ['2025030109', '柳岸', 'none', 280],
  ['2025030110', '蒋一苇', 'none', null],
  // 202504 班 · 两极分化
  ['2025040101', '骆云帆', 'top', 3],
  ['2025040102', '高见', 'top', 18],
  ['2025040103', '尹千帆', 'top', 60],
  ['2025040104', '童雨时', 'good', 100],
  ['2025040105', '简宁', 'low', 150],
  ['2025040106', '聂晓', 'low', 175],
  ['2025040107', '施然', 'starter', 205],
  ['2025040108', '单泠', 'starter', 250],
  ['2025040109', '谷雨', 'none', 320],
  ['2025040110', '满诚', 'none', null],
  // 202505 班 · 中段集中
  ['2025050101', '邵一粟', 'good', 8],
  ['2025050102', '田海宁', 'solid', 24],
  ['2025050103', '冯渡', 'solid', 44],
  ['2025050104', '尤佳悦', 'solid', 80],
  ['2025050105', '白景行', 'mid', 110],
  ['2025050106', '祁朗', 'mid', 140],
  ['2025050107', '兰澜', 'mid', 168],
  ['2025050108', '郁风', 'low', 220],
  ['2025050109', '时雨', 'starter', 290],
  ['2025050110', '苗青', 'none', null],
];

const QUESTIONS = STAGES.flatMap((s) =>
  s.questions.map((q) => ({
    stageId: s.id,
    qid: q.id,
...
function levelOf(profile: string, isTopClass: boolean): 'full' | 'mid' | 'low' {
...
/** 分页拉全量用户（注入按 email 复用账号、清除按 email 定位，都要全集） */
...
/** 学生互相独立，分批并行写库（50 人 8 并发可在十几秒内完成，避免请求超时） */
...
export async function seedDemoData(admin: SupabaseClient) {
  const byEmail = new Map((await listAllUsers(admin)).map((u) => [u.email, u.id]));
  let created = 0;
  let reused = 0;

  await inBatches(STUDENTS, 8, async ([staffNo, name, profileKey, hoursAgo]) => {
    ...
    if (clsName) {
      ...
      if (error) throw new Error(`${staffNo} 班级归属: ${error.message}`);
    }
    ...
    const { answerQ, submitQ, stages, revealCurrent, v, exhB, exhC } = profile;
    const level = levelOf(profileKey, isTopClass);
    const answers: Record<string, string> = {};
    const submitted: Record<string, boolean> = {};
    const verdicts: Record<string, string> = {};
    const images: Record<string, string> = {};
    const actsRevealed: Record<string, number> = {};
    ...
    // 鉴赏足迹按板块顺序推进，时间落在工序学习前后两小时内错落分布
    const EXH_BOARD_IDS = ['discovery', 'forms', 'themes', 'cases', 'reference'];
    const baseAt = Date.now() - (hoursAgo ?? 0) * 3600e3;
    const exhibitsViewed: Record<string, string> = {};
    for (let i = 0; i < exhB; i++) {
      exhibitsViewed[EXH_BOARD_IDS[i]] = new Date(baseAt - i * 20 * 60e3).toISOString();
    }
    for (let c = 1; c <= exhC; c++) {
      exhibitsViewed[`case-${c}`] = new Date(baseAt - (30 + c * 15) * 60e3).toISOString();
    }

    const { error: upErr } = await admin.from('workshop_progress').upsert(
      {
        user_id: userId,
  ...
  return { seeded: STUDENTS.length, created, reused };
}

/** 只清演示账号（@demo.invalid）：进度、班级归属、账号本体；真实学生不受影响 */
export async function clearDemoData(admin: SupabaseClient) {
  const demoUsers = (await listAllUsers(admin)).filter((u) => u.email?.endsWith('@demo.invalid'));
  await inBatches(demoUsers, 8, async (u) => {
    await admin.from('workshop_progress').delete().eq('user_id', u.id);
    await admin.from('class_assignments').delete().eq('user_id', u.id);
    const { error } = await admin.auth.admin.deleteUser(u.id);
    if (error) throw new Error(`${u.email}: ${error.message}`);
  });
  return { removed: demoUsers.length };
}
