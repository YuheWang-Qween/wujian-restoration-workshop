...
>
> **题型机制**：细问不限于问答，共六种作答形式（缺省=文本框）。小问数据带
> `WjPart.input`，无小问的整题（1-q2 判断题）用 `WjQuestion.input`：
> `ordering` 分层排序（`→` 分隔层、同层 `、` 并列表示先后不确定；点层选中后点单位入层，
> 层可增删/上移，附补充说明框，如 1-q1(a)——偏序关系必须允许同层多项，线性单链是错的）、
> `choice` 单选（如 2-q1(a)）、`multi` 多选（如 3-q2(b)、4-q1(b)）、
> `judge` 逐条正误判断（整题级，1-q2）、`matching` 左右匹配（5-q3(a)）；
...
> 组装题目时经 `describeInput()`（prompt.ts 导出、两边共用）把选项/待排项/判断条目/匹配列
> 注入题干（题干 text 不再含选项）。
