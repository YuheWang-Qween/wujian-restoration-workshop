...
> `input: { type: 'ordering', items }` 为排序题（点选入列 + ↑/× 微调 + 补充说明框，
> 如 1-q1(a)）、`input: { type: 'choice', options }` 为单选题（如 2-q1(a)，选项从题干
> 抽入 options）。结构化答案**序列化为纯文本协议**存入 answers（与文本题共用存储、
> 判对错与问小简通道）：排序题 `排序：a → b → c`（+ 可选 `补充：…` 行），单选题
> `选择：B`；chat 与判分的系统提示词都写有「格式约定」教模型读协议，且
> prompt.ts / grade-prompt.ts 组装题目时会把选项/待排项注入题干（题干 text 不再含选项）。
