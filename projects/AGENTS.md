...
> 环节页 header 下有「编绳串简」四节进度条（StageProgressStrip）：已读节墨色简片可点击
> 回跳（`revealToAct`），当前节朱砂呼吸，未到节虚线不可点；细问节带 `已答/总题` 子进度
> （口径与完成判定同源 `isQuestionAnswered`，store 导出、环节页与进度条共用）；
> 四片全墨后末节盖朱印「阅」。样式在 globals.css `.wj-strip*`。
